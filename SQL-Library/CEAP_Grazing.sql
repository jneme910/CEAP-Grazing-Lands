...
(SELECT TOP 1 desgnsuffix  FROM chorizon AS ch2 
INNER JOIN chdesgnsuffix AS chs ON chs.chkey=ch2.chkey AND chorizon.chkey=chs.chkey AND desgnsuffix = 's') AS spodic_flag
FROM #comp
...
min_top_depth , prev_texture_grouping,   prev_bottom_depth,  row_num  From #surface_tex3 WHERE 
 ...
 --link
 SELECT DISTINCT cokey,  compname, tex_modifier, tex_in_lieu, texture_grouping,	 hz_diag_kind,  
  min_top_depth ,  MAX(hzdepb_r) over(partition by cokey) as max_bottom_depth 
 INTO #surface_final
 FROM #surface_tex4
...
SELECT areasymbol, areaname,  musym, #map.mukey, muname, #comp.compname, #comp.cokey, #comp.comppct_r , #comp.landform, #comp.min_yr_water, #horizon.subgroup, #horizon.greatgroup, avg_h20_apr2sept , avg_h20_oct2march,  max_ec_profile, max_sar_profile, maxcaco3_0_2cm, maxcaco3_2_13cm, maxcaco3_13_50cm, maxsar_0_2cm, maxsar_2_13cm, maxsar_13_50cm, spodic_flag, #surface.hzname  , #surface.hzdept_r , #surface.hzdepb_r , #surface.texture, #surface.mineral_des, #surface.om_r, #surface.surface_mineral, 
#surface_final.tex_modifier, #surface_final.tex_in_lieu, #surface_final.texture_grouping, #surface_final.hz_diag_kind,   #surface_final.min_top_depth , #surface_final.max_bottom_depth ,
...
#rest.[Densic bedrock],#rest.[Lithic bedrock],#rest.[Paralithic bedrock],#rest.[Cemented horizon],#rest.[Duripan],#rest.[Fragipan],#rest.[Manufactured layer],#rest.[Petrocalcic],#rest.[Petroferric],[Petrogypsic], ---rest 
